...
def readCommand(argv):
    ...
    parser.add_option('-n', '--numGames', dest='numGames', type='int',
                      help=default('the number of GAMES to play'), metavar='GAMES', default=200000)
    parser.add_option('-l', '--layout', dest='layout',
                      help=default(
                          'the LAYOUT_FILE from which to load the map layout'),
                      metavar='LAYOUT_FILE', default='smallGrid')
    # smallGrid mediumClassic medium2times
    ...
    parser.add_option('-x', '--numTraining', dest='numTraining', type='int',
                      help=default('How many episodes are training (suppresses output)'), default=199000)
...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    ...
    if (numGames - numTraining) > 0:
        # 绘制训练过程中的mean_reward曲线
        # 假设这是你的 list 数据
        data = pacman.mean_reward
        import json
        # # 保存为 JSON 文件
        # with open('mean_reward.json', 'w') as file:
        #     json.dump(data, file)
        # 创建 x 轴数据（索引）
        # x = range(len(data))
        # # 绘制曲线图
        # plt.plot(x, data, label='Mean_Reward')
        # plt.text(x[-1],data[-1],' %f' % data[-1], ha='left', va='center', fontsize=10)
        # # 添加标题和标签
        # plt.title('mean_reward / 100 episodes')
        # plt.xlabel('Index')
        # plt.ylabel('Reward')
        # # 显示图例
        # plt.legend()
        ...
        # plt.tight_layout()
        # # 保存图像到文件
        # plt.savefig("result_pic/train_mediumClassic_SNN_youf4linear_model_1", dpi=300, bbox_inches='tight')  # dpi 设置图像分辨率
        # print(f"图像已保存到save_pic")
        # # 显示图表
        # plt.show()

        # data = pacman.mean_score
        # # with open('mean_score.json', 'w') as file:
        # #     json.dump(data, file)
        # # 创建 x 轴数据（索引）
        # x = range(len(data))
        # # 绘制曲线图
        # plt.plot(x, data, label='Mean_Score')
        # plt.text(x[-1],data[-1],' %f' % data[-1], ha='left', va='center', fontsize=10)
        # # 添加标题和标签
        # plt.title('mean_score / 100 episodes')
        # plt.xlabel('Index')
        # plt.ylabel('Score')
        # # 显示图例
        # plt.legend()
        ...
        # plt.tight_layout()
        # # 保存图像到文件
        # plt.savefig("result_pic/train_mediumClassic_SNN_youf4linear_model_2", dpi=300, bbox_inches='tight')  # dpi 设置图像分辨率
        # print(f"图像已保存到save_pic")
        # # 显示图表s
        # plt.show()
