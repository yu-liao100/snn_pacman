...
class PacmanDQN(PacmanUtils):
    def __init__(self, args):        
        ...
        if(model_trained == True):
            ...
            # print("进来了")
            # return
            self.policy_net = DQN_change().to(self.device)
            self.target_net = DQN_change().to(self.device)
            # self.policy_net = DQN_SNN_SingleTimeSteps().to(self.device)
            # self.target_net = DQN_SNN_SingleTimeSteps().to(self.device)
            # self.policy_net = DQN_SNN().to(self.device)
            # self.target_net = DQN_SNN().to(self.device)
        ...
        self.last_mem2 = 0
        self.last_mem3 = 0
        self.current_mem1 = 0 # 当前时刻的膜电位
        self.current_mem2 = 0
        self.current_mem3 = 0
        self.hidden = None
        self.last_move = None
          # self.h = torch.zeros(1, 1, 4, device= "cuda:4", dtype=torch.float64)
        self.h = torch.zeros(1, 1, 512 ,device= "cuda:4", dtype=torch.float64)
        self.c = torch.zeros(1, 1, 512, device= "cuda:4", dtype=torch.float64)
        self.lasth = torch.zeros(1, 1, 512 ,device= "cuda:4", dtype=torch.float64)
        self.lastc = torch.zeros(1, 1, 512 ,device= "cuda:4", dtype=torch.float64)
    ...
    def getMove(self, state): # epsilon greedy
        ...
        if random_value > self.epsilon: # exploit 
            ...
            if isinstance(self.policy_net, DQN):
            ...
            elif isinstance(self.policy_net, DQN_change):
                self.Q_found,self.h,self.c = self.policy_net(temp_current_state,self.lasth,self.lastc)
            else:
                ...
                self.Q_found,self.h,self.c = self.policy_net(temp_current_state,self.lasth,self.lastc)
                # print("self.Q_found",self.Q_found)
                # 等比例缩小残余电位看看影响
                # self.last_mem1 *= 0.1
                # self.last_mem2 *= 0.1
                # self.last_mem3 *= 0.1
    ...
    def observation_step(self, state):
        if self.last_action is not None:
            ...
            if isinstance(self.policy_net, DQN):
                ...
                transition = (self.last_state, self.last_reward, self.last_action, self.current_state, self.terminal,self.lasth,self.lastc)
                self.lasth = self.h
                self.lastc = self.c
            else:
                if config.epsilon_mem:
                    ...
                    transition = (self.last_state, self.last_reward, self.last_action, self.current_state, self.terminal,self.last_mem1,self.last_mem2,self.last_mem3)
                    self.last_mem1 = self.current_mem1
                    self.last_mem2 = self.current_mem2
                    self.last_mem3 = self.current_mem3
            ...
            if config.is_gameover:
                ...
                # self.last_mem2 = 0
                # self.last_mem3 = 0
                # self.policy_net.spike_1.mem = 0
                # self.policy_net.spike_2.mem = 0
                # self.policy_net.spike_3.mem = 0
                # self.target_net.spike_1.mem = 0
                # self.target_net.spike_2.mem = 0
                # self.target_net.spike_3.mem = 0
    ...
    def final(self, state):
        ...
        #     # save model
        #     torch.save(self.policy_net, f'smallGrid_测试ANN的DRQN/pacman_policy_net_{self.counter}.pt')
        #     torch.save(self.target_net, f'smallGrid_测试ANN的DRQN/pacman_target_net_{self.counter}.pt')
    ...
    def train(self):
        if (self.local_cnt > start_training):
            ...
            if isinstance(self.policy_net, DQN):
                ...
                batch_s, batch_r, batch_a, batch_n, batch_t, batch_h, batch_c = zip(*batch)
            else:
                batch_s, batch_r, batch_a, batch_n, batch_t, batch_mem1, batch_mem2, batch_mem3 = zip(*batch)
            ...
            if isinstance(self.policy_net, DQN):
                ...
                batch_h = torch.stack(batch_h,dim = 1).squeeze(2)
                batch_c = torch.stack(batch_c,dim = 1).squeeze(2)
            else:
            ...
            if isinstance(self.policy_net, DQN):
                ...
                # h = torch.zeros(1, 32, 4, device= "cuda:4", dtype=torch.float64)
                # h = torch.zeros(1, batch_size, 512, device= "cuda:4", dtype=torch.float64)
                # c = torch.zeros(1, batch_size, 512, device= "cuda:4", dtype=torch.float64)
                # print("batch_a.shape",batch_a.shape)
                # print("self.policy_net(batch_s,h,c)[0].shape",self.policy_net(batch_s,h)[0].shape)
                # batch_a = batch_a.unsqueeze(2)
                # print()
                state_action_values = self.policy_net(batch_s,batch_h,batch_c)[0].gather(1, batch_a)
            else:
                state_action_values = self.policy_net(batch_s)[0].gather(1, batch_a)
            ...
            if isinstance(self.policy_net, DQN):
                ...
                # h = torch.zeros(1, batch_size, 512, device= "cuda:4", dtype=torch.float64)
                # c = torch.zeros(1, batch_size, 512, device= "cuda:4", dtype=torch.float64)
                next_state_values = self.target_net(batch_n,batch_h,batch_c)[0]
            else:
                ...
                # self.target_net.spike_2.mem = 0
                # self.target_net.spike_3.mem = 0
                next_state_values = self.target_net(batch_n)[0]
