...
class PacmanDQN(PacmanUtils):
    def __init__(self, args):        
        ...
        if(model_trained == True):
            ...
            self.policy_net = DQN_SNN_SingleTimeSteps_changev2().to(self.device)
            self.target_net = DQN_SNN_SingleTimeSteps_changev2().to(self.device)
    ...
    def getMove(self, state): # epsilon greedy
        ...
        if random_value > self.epsilon: # exploit 
            ...
            if isinstance(self.policy_net, DQN):
                ...
                self.Q_found,self.last_mem1,self.last_mem2,self.last_mem3 = self.policy_net(temp_current_state,self.last_mem1,self.last_mem2,self.last_mem3)
                # 等比例缩小残余电位看看影响
                # self.last_mem1 *= 0.1
                # self.last_mem2 *= 0.1
                # self.last_mem3 *= 0.1
    ...
    def train(self):
        if (self.local_cnt > start_training):
            ...
            if isinstance(self.policy_net, DQN):
                ...
                state_action_values = self.policy_net(batch_s,batch_mem1,batch_mem2,batch_mem3)[0].gather(1, batch_a)
            ...
            if isinstance(self.policy_net, DQN):
                ...
                next_state_values = self.target_net(batch_n,batch_mem1,batch_mem2,batch_mem3)[0]
