...
exp_name = "smallGrid_SNN_部分观测r=1"
...
class PacmanDQN(PacmanUtils):
    ...
    def getMove(self, state): # epsilon greedy
        ...
        if random_value > self.epsilon: # exploit 
            ...
            if isinstance(self.policy_net, DQN):
            ...
            elif isinstance(self.policy_net, DQN_SNN):
                self.Q_found = self.policy_net(temp_current_state)
            else:
                ...
                self.Q_found,self.current_mem1,self.current_mem2,self.current_mem3 = self.policy_net(temp_current_state)
                # print("self.Q_found",self.Q_found.shape)
                # 等比例缩小残余电位看看影响
                # self.last_mem1 *= 0.1
                # self.last_mem2 *= 0.1
                # self.last_mem3 *= 0.1
    ...
    def final(self, state):
        ...
        if(self.counter % 100 == 0):
            ...
            # logger.update(score = self.sum_reward,total_steps = self.episode_number)
    ...
    def train(self):
        if (self.local_cnt > start_training):
            ...
            if isinstance(self.policy_net, DQN):
                ...
                # print("self.policy_net(batch_s)[0].shape",self.policy_net(batch_s)[0].shape)
