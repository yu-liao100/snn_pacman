...
class Agent:
    ...
    def energyCost(self, cost_energy):
        """
        计算每次行动的能量消耗
        """
        return cost_energy

    def initialDelay(self,time):
        return time  # 第一次行动的延迟时间，单位可以是秒或步

    def actionDelay(self,time):
        return time  # 每次行动后的延迟时间
...
class Game:
    ...
    def __init__(self, agents, display, rules, startingIndex=0, muteAgents=False, catchExceptions=False):
        ...
        ## 设置全局时钟
        self.eventQueue = []  # 优先级队列，存储 (时间戳, agentIndex, action)
        self.currentTime = 0  # 全局时钟
    
    # def scheduleNextAction(self, agentIndex, delay):
    #     """
    #     调度下一次行动（如果能量足够）
    #     """
    ...
    #         # 检查能量是否足够
    #         if config.total_energy <= 0:
    #             # print(f"Agent {agentIndex} has run out of energy and will stop.")
    #             return  # 停止调度
    #         else:
    #             # config.total_energy -= 1
    #             config.total_energy -= config.snn_single_action_consumption
    #             # print(config.ann_single_action_consumption)
    #             config.snn_single_action_consumption = 0
    #             # print(f"Agent {agentIndex} performed an action. Energy left: {config.total_energy:.2f}")

    #             """
    #             根据 agentIndex 和延迟时间 delay，调度下一次动作
    #             """
    #             # print(self.currentTime)
    #             nextTime = self.currentTime + delay
    #             heapq.heappush(self.eventQueue, (nextTime, agentIndex))
    #     else:
    #         """
    #         根据 agentIndex 和延迟时间 delay，调度下一次动作
    #         """
    ...
    def run(self):
        ...
        if config.is_asynchronous:
            for agentIndex in range(len(self.agents)):
                ...
                # 如果是pacman
                if 'observationFunction' in dir(agent):
                    self.scheduleNextAction(agentIndex, agent.initialDelay(1))
                else:
                    self.scheduleNextAction(agentIndex,agent.initialDelay(1))
        
            while not self.gameOver and self.eventQueue:
            # while not self.gameOver:
                # 取出最近的事件
                self.currentTime, agentIndex = heapq.heappop(self.eventQueue)
                ...
                if not self.gameOver:
                    if 'observationFunction' in dir(agent):
                        # 为 agent 安排下一次行动
                        self.scheduleNextAction(agentIndex, agent.actionDelay(1)) # SNN是0.125
                    else:
                        self.scheduleNextAction(agentIndex, agent.actionDelay(1))
                # Change the display
        ...
        # print("死了")
        if self.gameOver:
            config.total_energy = 2.0
            config.is_gameover = True
